<template>
  ...
  <div class="calculations-wrapper">
      <div class="calculations-input">
          ...
          :invalid="v$.calculationsValue.$dirty && v$.calculationsValue.$invalid"
          />
      </div>
...
export default {
    ...
    methods: {
        add(target){
            ...
            this.$store.state[target].push(payload)
            this.notif.success({
        ...
        async addAndSave(target){
            ...
            if(!this.v$.$invalid){
              this.v$.$reset()
              const addedPayload = await this.add(target)
              saveData({ target: target, payload: addedPayload})
            }  
        },
    },
